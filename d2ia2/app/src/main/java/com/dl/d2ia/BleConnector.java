...
//    public static UUID RX_UUID = UUID.fromString("6E400003-B5A3-F393-E0A9-E50E24DCCA9E");
    ...
    public BleConnector(Context c, Handler h) {
        mContext = c;
        mHandler = h;
        mBluetoothAdapter = BluetoothAdapter.getDefaultAdapter();
    }

    public BleConnector(Context c) {
        mContext = c;
    }
    ...
    private BluetoothGattCallback callback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (newState == BluetoothGatt.STATE_CONNECTED) {
                ...
                mHandler.obtainMessage(HandleId.connectState, "成功连接到目标设备".getBytes()).sendToTarget();
            } else if (newState == BluetoothGatt.STATE_DISCONNECTED) {
                writeLine("Disconnected!");
                mConnectionState = STATE_DISCONNECTED;
                mHandler.obtainMessage(HandleId.connectState, "断开连接".getBytes()).sendToTarget();
            } else if (newState == BluetoothGatt.STATE_CONNECTING) {
                writeLine("Connecting!");
                mHandler.obtainMessage(HandleId.connectState, "连接中".getBytes()).sendToTarget();
            } else {
...
//                    }
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            ...
            mHandler.obtainMessage(HandleId.onCharacteristicChanged, hexString.getBytes()).sendToTarget();
